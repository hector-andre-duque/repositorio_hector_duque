...
public class AdministracionParqueaderoImpl implements AdministracionParqueadero {
	...
	public void registrarEntrada(String nroPlaca) {						
			...
			if (vehiculo.getEstadoVehiculo()== EstadoVehiculo.NO_RESIDENTE && vehiculo.getTipoVehiculo() == TipoVehiculo.PARTICULAR) {
	...
	public RegistroCobro registrarSalida(Vehiculo vehiculo) {
		...
		if (vehiculo.getEstadoVehiculo()== EstadoVehiculo.NO_RESIDENTE && vehiculo.getTipoVehiculo() == TipoVehiculo.PARTICULAR ) {				
			...
			RegistroNoResidente registroNoResidente = registroParqueDAO.buscarRegistroNoResidente(vehiculo.getNroPlaca());
			...
			minutos =   registroResidente.getTiempoEstacionado() + getDiferenciaMinutosDate(registroResidente.getFechaIngreso(), fechaActual);
			if (minutos == 0) {
				minutos = 1;
			}		
			...
			registroOficial.setEstancias(registrarEstanciasOficial(registroOficial.getFechaIngreso(),fechaActual,registroOficial.getEstancias()));
	...
	public String guardarFicheroResidentes(String nombreArchivo) {
		...
		try { 
			...
			for (RegistroResidente registroResidente : listResidentes) {
				bfwriter.newLine();
				bfwriter.write(registroResidente.getNroPlaca() + "				" + registroResidente.getTiempoEstacionado()
